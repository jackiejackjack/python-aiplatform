...
def save_model(
    ...
    *,
    ...
    """Save the ML model into a MLMD artifact.
    ...
    Args:
        ...
        artifact_id (str):
            ...
            It may be up to 63 characters, and valid characters are `[a-z0-9_-]`. The first character cannot be a number or hyphen.
        uri (str):
...
def _save_sklearn_model(
    ...
    """Save a sklearn model into a MLMD artifact.

    Args:
        model (sklearn.base.BaseEstimator):
            Requred. A sklearn model object.
        artifact_id (str):
            Optional. The resource id of the artifact. This id must be globally unique in a metadataStore.
        uri (str):
            Optional. A gcs directory to save the model file.
...
) -> Union[sklearn.base.BaseEstimator, tf.Module, xgb.Booster]:
    """Retrieves the original model instance from the model saved in Vertex.
