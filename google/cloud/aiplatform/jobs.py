...
class BatchPredictionJob(_Job):
        ...
        if explanation_metadata or explanation_parameters:
            ...
            if model_monitoring_objective_config:
                ...
                gapic_batch_prediction_job.explanation_spec = (
                    gca_explanation_v1beta1.ExplanationSpec.deserialize(
                        gca_explanation_compat.ExplanationSpec.serialize(
                            explanation_spec
                        )
                    )
                )
    ...
    def _create(
        ...
        if (
            ...
            and model_monitoring_objective_config.skew_detection_config
        ):
        ...
        if (
            ...
            model_monitoring_objective_config.skew_detection_config._config_for_bp = (
                True
            )
            ...
            if model_monitoring_objective_config.drift_detection_config:
                _LOGGER.info(
                    "Drift detection config is currently not supported for monitoring models associated with batch prediction jobs. It will be dropped in the current config."
                )
            if model_monitoring_objective_config.explanation_config:
                _LOGGER.info(
                    "XAI config is currently not supported for monitoring models associated with batch prediction jobs. It will be dropped in the current config"
                )
            ...
            gca_batch_prediction_job.model_monitoring_config = gapic_mm_config
